...
export class AuthApi {
  public static path = process.env['API_ENDPOINT'];

  public static async signIn(data: SignInRequest): Promise<SignInResponseData> {
    const res = await HTTPTransport.post(`${AuthApi.path}/auth//signin`, {
      data,
      headers: {
  ...
  public static async signUp(data: SignUpRequest): Promise<SignUpResponseData> {
    const res = await HTTPTransport.post(`${AuthApi.path}/auth/signup`, {
      data,
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  public static async user(): Promise<UserResponseData> {
    const res = await HTTPTransport.get(`${AuthApi.path}/auth/user`, {
      headers: {
        'Content-Type': 'application/json',
      },
    });
  ...
  public static async logout(): Promise<SignUpResponseData> {
    const res = await HTTPTransport.post(`${AuthApi.path}/auth/logout`, {
      headers: {
        'Content-Type': 'application/json',
      },
    });
